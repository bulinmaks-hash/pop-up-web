...
results.floatingCartInitiallyHidden = await evaluate("document.querySelector('.floating-cart') === null");
...
results.floatingCartHiddenAfterScroll = await evaluate("document.querySelector('.floating-cart') === null");
...
const catalogTarget = await evaluate("document.querySelector('.product-popup').getBoundingClientRect().top + scrollY - innerHeight * .5");
await evaluate("window.scrollTo(0," + catalogTarget + ")");
await wait(260);
results.catalogEntryAngle = await evaluate("Number(document.querySelector('.product-popup').dataset.openAngle)");
results.cardAutoOpened = await evaluate("document.querySelector('.product-popup').getAttribute('aria-expanded') === 'true'");
await evaluate("document.querySelector('.product-popup').click()");
await wait();
results.cardManualClosed = await evaluate("document.querySelector('.product-popup').getAttribute('aria-expanded') === 'false'");
await evaluate("document.querySelector('.product-popup').click()");
await wait();
results.cardManualReopened = await evaluate("document.querySelector('.product-popup').getAttribute('aria-expanded') === 'true'");
...
results.floatingCartWidth = await evaluate("Math.round(document.querySelector('.floating-cart').getBoundingClientRect().width)");
...
await evaluate("document.querySelector('.quantity button:first-child').click()");
await wait();
await evaluate("document.querySelector('.quantity button:first-child').click()");
await wait();
results.floatingCartHiddenAfterEmptying = await evaluate("document.querySelector('.floating-cart') === null");
...
const expected = results.initialProducts === 3 && results.floatingCartInitiallyHidden && results.floatingCartHiddenAfterScroll && results.floatingCartPosition === "fixed" && results.floatingCartVisible && results.floatingCartWidth <= 124 && results.heroInitialAngle === 160 && results.heroClosedAngle < results.heroInitialAngle && results.heroClosedAngle > results.heroExitAngle && results.heroClosedFold > results.heroInitialFold && results.heroExitAngle <= 3 && results.heroExitFold >= 87 && results.heroExitBottom <= 1 && Math.abs(results.heroClosedHinge - results.heroInitialHinge) <= 1 && results.heroLightSource === "top-left" && results.heroClosedRightShade > results.heroInitialRightShade && results.heroClosedShadowWidth < results.heroInitialShadowWidth && results.heroReopenedAngle > results.heroClosedAngle && results.filteredProducts === 1 && results.catalogEntryAngle >= 159 && results.cardAutoOpened && results.cardManualClosed && results.cardManualReopened && results.cartCount === "1" && results.drawerOpen && results.cartItems === 1 && results.quantity === "2" && results.floatingCartHiddenAfterEmptying && results.subscribed && results.browserErrors.length === 0;
